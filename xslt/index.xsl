            ...
            <body class="page" style="background-color:#f1f1f1;">
                ...
                <script>
                    ...
                    function displayOverviewSlide() {
                        ...
                        html += '<p class="mb-1"><strong>Zeitraum</strong></p>';
                        if (statsData.date_range && statsData.date_range.earliest && statsData.date_range.latest) {
                            html += '<p class="mb-0">' + formatGermanDate(statsData.date_range.earliest) + ' bis ' + formatGermanDate(statsData.date_range.latest) + '</p>';
                        }
                        html += '</div>';

                        // Object types
                        html += '<div class="col-md-8">';
                        html += '<p class="mb-1"><strong>Objekttypen</strong></p>';
                        if (statsData.by_object_type) {
                            const types = Object.entries(statsData.by_object_type);
                            const typeStrings = types.map(([type, count]) => {
                                const firstLetter = type.charAt(0).toUpperCase();
                                return firstLetter + '\u00A0(' + count + ')';
                            });
                            html += '<p class="mb-0">' + typeStrings.join(', ') + '</p>';
                        }
                        html += '</div>';
