...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
    ...
    <xsl:variable name="quotationString">
        <xsl:value-of
            select="concat(normalize-space(//tei:titleStmt/tei:title[@level = 'a']), '. In: Arthur Schnitzler: Briefwechsel mit Autorinnen und Autoren. Digitale Edition. Herausgeben von Martin Anton Müller mit Gerd Hermann Susen, Laura Untner und Selma Jahnke, ', $quotationURL, ' (Abfrage ', $currentDate, ')')"
        />
    </xsl:variable>
    ...
    <xsl:template match="/">
        ...
        <html xmlns="http://www.w3.org/1999/xhtml" style="hyphens: auto;" lang="de" xml:lang="de">
            <head>
                ...
                <!-- Enhanced SEO Meta Tags for Letters -->
                <xsl:variable name="letter_description">
                    ...
                    <xsl:if test="//tei:titleStmt/tei:title[@type='iso-date']/@when-iso">
                        ...
                        <xsl:value-of select="format-date(xs:date(//tei:titleStmt/tei:title[@type='iso-date']/@when-iso), '[D1]. [MNn] [Y]', 'de', (), ())"/>
                    </xsl:if>
                    <xsl:text>. Teil der digitalisierten Korrespondenz Arthur Schnitzlers.</xsl:text>
                </xsl:variable>
                
                <meta name="description" content="{normalize-space($letter_description)}"/>
                
                <!-- Keywords based on entities in the letter -->
                <xsl:variable name="letter_keywords">
                    <xsl:text>Arthur Schnitzler, Briefwechsel, Korrespondenz</xsl:text>
                ...
                <meta name="keywords" content="{normalize-space($letter_keywords)}"/>
                
                <!-- OpenGraph Meta Tags -->
                <meta property="og:title" content="{//tei:titleStmt/tei:title[@level = 'a']}"/>
                <meta property="og:description" content="{normalize-space($letter_description)}"/>
                <meta property="og:type" content="article"/>
                <meta property="og:url" content="https://schnitzler-briefe.acdh.oeaw.ac.at/{$link}"/>
                <meta property="og:site_name" content="Arthur Schnitzler: Briefwechsel mit Autorinnen und Autoren"/>
                <meta property="og:locale" content="de_AT"/>
                
                <xsl:if test="descendant::tei:pb[not(@facs = '')][1]/@facs">
                    <xsl:variable name="iiif-ext" select="'.jpg?format=iiif&amp;param=/full/1200,630/0/default.jpg'"/>
                    ...
                    <meta property="og:image" content="{concat($iiif-domain, $facs_item, $iiif-ext)}"/>
                    <meta property="og:image:width" content="1200"/>
                    <meta property="og:image:height" content="630"/>
                    <meta property="og:image:alt" content="Faksimile des Briefes: {//tei:titleStmt/tei:title[@level = 'a']}"/>
                </xsl:if>
                
                <!-- Twitter Card Meta Tags -->
                <meta name="twitter:card" content="summary_large_image"/>
                <meta name="twitter:title" content="{//tei:titleStmt/tei:title[@level = 'a']}"/>
                <meta name="twitter:description" content="{normalize-space($letter_description)}"/>
                ...
                <xsl:if test="descendant::tei:pb[not(@facs = '')][1]/@facs">
                    ...
                    <xsl:variable name="iiif-domain" select="'https://iiif.acdh-dev.oeaw.ac.at/iiif/images/schnitzler-briefe/'"/>
                    <xsl:variable name="facs_item" select="descendant::tei:pb[not(@facs = '')][1]/@facs"/>
                    <meta name="twitter:image" content="{concat($iiif-domain, $facs_item, $iiif-ext)}"/>
                    <meta name="twitter:image:alt" content="Faksimile des Briefes: {//tei:titleStmt/tei:title[@level = 'a']}"/>
                </xsl:if>
                
                <!-- Article specific meta tags -->
                <xsl:if test="//tei:titleStmt/tei:title[@type='iso-date']/@when-iso">
                    <meta property="article:published_time" content="{//tei:titleStmt/tei:title[@type='iso-date']/@when-iso}"/>
                </xsl:if>
                <meta property="article:author" content="Arthur Schnitzler"/>
                <meta property="article:section" content="Briefwechsel"/>
                
                <!-- Canonical URL for this specific letter -->
                <link rel="canonical" href="https://schnitzler-briefe.acdh.oeaw.ac.at/{$link}"/>
                ...
                <script type="application/ld+json">
                {
                    ...
                    "name": "<xsl:value-of select="//tei:titleStmt/tei:title[@level = 'a']"/>",
                    "description": "<xsl:value-of select="normalize-space($letter_description)"/>",
                    "url": "https://schnitzler-briefe.acdh.oeaw.ac.at/<xsl:value-of select="$link"/>",
                    ...
                    "author": {
                        "@type": "Person",
                        "name": "<xsl:value-of select="//tei:correspAction[@type='sent']//tei:persName[1]"/>"
                    },
                    </xsl:if>
                    <xsl:if test="//tei:correspAction[@type='received']//tei:persName">
                    "recipient": {
                        "@type": "Person", 
                        "name": "<xsl:value-of select="//tei:correspAction[@type='received']//tei:persName[1]"/>"
                    },
                    </xsl:if>
                    "isPartOf": {
                        "@type": "DigitalCollection",
                        "name": "Arthur Schnitzler: Briefwechsel mit Autorinnen und Autoren",
                        "description": "Digitale Edition der Korrespondenz Arthur Schnitzlers",
                    ...
                    "keywords": [
                        ...
                        "Literatur",
                        "Wien",
                        "Fin de Siècle"<xsl:for-each select="descendant::tei:back/tei:listPerson/tei:person[position() &lt;= 5]">,
                        "<xsl:value-of select="concat(./tei:persName/tei:surname, ', ', ./tei:persName/tei:forename)"/>"</xsl:for-each>
                    ]<xsl:if test="descendant::tei:pb[not(@facs = '')][1]/@facs">,
                    "image": {
                        "@type": "ImageObject",
                        "url": "<xsl:value-of select="concat('https://iiif.acdh-dev.oeaw.ac.at/iiif/images/schnitzler-briefe/', descendant::tei:pb[not(@facs = '')][1]/@facs, '.jpg?format=iiif&amp;param=/full/1200,630/0/default.jpg')"/>",
                        "description": "Faksimile des Briefes"
                    }</xsl:if>
                }
                </script>
